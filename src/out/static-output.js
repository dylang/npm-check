...
function render(pkg, currentState) {
  ...
  const upgradeMessage = `${chalk.green(
    upgradeCommand,
  )} to go from ${pkg.installed} to ${pkg.latest}`;
  // DYLAN: clean this up
  const status = _([
    pkg.notInstalled
      ? `${chalk.bgRed.white.bold(
        `${emoji(' :worried: ')} MISSING! `,
      )} Not installed.`
      : '',
    pkg.notInPackageJson
      ? `${chalk.bgRed.white.bold(
        `${emoji(' :worried: ')} PKG ERR! `,
      )} Not in the package.json. ${pkg.notInPackageJson}`
      : '',
    ...
    pkg.unused
      ? [
        ...
        `${indent}Depcheck did not find code similar to ${chalk.green(
          `require('${packageName}')`,
    ...
    pkg.mismatch && !pkg.bump
      ? `${chalk.bgRed.yellow.bold(
        `${emoji(' :interrobang: ')} MISMATCH `,
      )} Installed version does not match package.json. ${pkg.installed} ≠ ${pkg.packageJson}`
      : '',
    pkg.regError
      ? `${chalk.bgRed.white.bold(
...
export default function outputConsole(currentState) {
  ...
  const rows = packages
    .reduce((acc, pkg) => acc.concat(render(pkg, currentState)), [])
    .filter(Boolean);
