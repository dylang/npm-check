...
function createPackageSummary(moduleName, currentState) {
    ...
    const modulePackageJson = readPackageJson(
    ...
    const packageJsonVersion =
        cwdPackageJson.dependencies[moduleName] ||
    ...
    if (ignore) {
        const ignoreMatch = Array.isArray(ignore)
            ? ignore.some(ignoredModule => minimatch(moduleName, ignoredModule))
    ...
    function foundIn(files) {
        ...
        return (
            'Found in: ' +
            files
                .map(filepath => filepath.replace(currentState.get('cwd'), ''))
                .join(', ')
        );
    }
    ...
    return getLatestFromRegistry(moduleName).then(fromRegistry => {
        ...
        const usingNonSemver =
            semver.valid(latest) && semver.lt(latest, '1.0.0-pre');

        const bump =
            semver.valid(latest) &&
            semver.valid(versionToUse) &&
            (usingNonSemver && semverDiff(versionToUse, latest)
                ? 'nonSemver'
                : semverDiff(versionToUse, latest));

        const unused = _.includes(unusedDependencies, moduleName);

        const latestStableReleaseTimestamp = differenceInCalendarYears(
            new Date(),
            new Date(fromRegistry.latestStableReleaseTimestamp),
        );
        ...
        return {
            ...
            unused: unused,
            latestStableReleaseTimestamp: latestStableReleaseTimestamp,
        };
    });
}
