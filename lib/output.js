...
function render(packageName, data) {
    ...
    // DYLAN: clean this up
    const status = _([
        data.notInstalled ? chalk.bgGreen.white.bold(emoji(' :worried: ') + ' MISSING! ') + ' Not installed.' : '',
        data.pkgError && !data.notInstalled ? chalk.bgGreen.white.bold(emoji(' :worried: ') + ' PKG ERR! ') + ' ' + chalk.red(data.pkgError.message) : '',
        ...
        data.bump && !data.easyUpgrade ? [
            chalk.white.bold.bgGreen((data.bump === 'nonSemver' ? emoji(' :sunglasses: ') + ' new ver! '.toUpperCase() : emoji(' :sunglasses: ') + ' ' + data.bump.toUpperCase() + ' UP ')) + ' ' + uppercaseFirstLetter(data.bump) + ' update available. ' + chalk.blue.underline(data.homepage || ''),
        ...
        data.regError ? chalk.bgRed.white.bold(emoji(' :no_entry: ') + ' NPM ERR! ') + ' ' + chalk.red(data.regError) : ''
    ])
    .flatten()
    .compact()
    .valueOf();
