...
function render(moduleName, data) {
    ...
    var status = _([
            !data.isInstalled ? chalk.bgRed.white.bold(' ' + emoji.tired_face + '  MISSING! ') + ' In package.json but not installed.' : '',
            ...
            data.bump && !data.easy_upgrade ? [
                chalk.black.bold.bgYellow(' ' + (data.bump === 'nonSemver' ? ' unknown up '.toUpperCase() : emoji.sunglasses + '  ' + data.bump.toUpperCase() + ' UP ')) + ' ' + uppercaseFirstLetter(data.bump) + ' update available: '  + chalk.blue.underline(data.homepage),
