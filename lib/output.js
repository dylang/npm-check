...
function outputConsole(modules) {
    if (options.get('debug')) {
        console.log('[npm-check debug] modules', modules);
    }
    ...
    const rows = _(modules).reduce((acc, data, moduleName) => {
    ...
    if (rows.length) {
        ...
        console.log('');
        console.log(t);
        console.log('Use ' + chalk.green('npm-check -u' + (options.get('global') ? ' -g' : '')) + ' for interactive update.');
        process.exit(-1);
    } else {
