...
function render(packageName, data) {
    ...
    const status = _([
        ...
        data.bump && data.easyUpgrade ? [
            ...
            '              ' + chalk.green('npm install ' + packageName) + ' to go from ' + data.installed + ' to ' + data.latest
        ] : '',
        data.bump && !data.easyUpgrade ? [
            ...
            '              ' + chalk.green('npm install --save' + (data.devDependency ? '-dev' : '') + ' ' + packageName + '@' + data.latest) + ' to go from ' + data.installed + ' to ' + data.latest
        ] : '',
        data.unused ? [chalk.black.bold.bgWhite(' ' + emoji('confused') + '  NOTUSED? ') + ' Possibly not referenced in the code.',
            '              ' + chalk.green('npm uninstall --save' + (data.devDependency ? '-dev' : '') + ' ' + packageName) + ' to remove.'
        ] : '',
