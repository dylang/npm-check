...
function render(pkg, currentState) {
    ...
    const upgradeMessage = `${chalk.green(upgradeCommand)} to go from ${
        pkg.installed
    } to ${pkg.latest}`;
    // DYLAN: clean this up
    const status = _([
        pkg.notInstalled
            ? chalk.bgRed.white.bold(emoji(' :worried: ') + ' MISSING! ') +
              ' Not installed.'
            : '',
        pkg.notInPackageJson
            ? chalk.bgRed.white.bold(emoji(' :worried: ') + ' PKG ERR! ') +
              ' Not in the package.json. ' +
              pkg.notInPackageJson
            : '',
        pkg.pkgError && !pkg.notInstalled
            ? chalk.bgGreen.white.bold(emoji(' :worried: ') + ' PKG ERR! ') +
              ' ' +
        ...
        pkg.unused
            ? [
                  ...
                  indent +
                      `Use ${chalk.green('--skip-unused')} to skip this check.`,
                  indent +
        ...
        pkg.mismatch && !pkg.bump
            ? chalk.bgRed.yellow.bold(emoji(' :interrobang: ') + ' MISMATCH ') +
            ...
            : '',
        pkg.regError
            ? chalk.bgRed.white.bold(emoji(' :no_entry: ') + ' NPM ERR! ') +
              ' ' +
              chalk.red(pkg.regError)
            : '',
        pkg.latestStableReleaseTimestamp >= 2 &&
            chalk.bgRed.white.bold(
                emoji(' :face_with_head_bandage: ') +
                    ' The last stable release for this dependency was published more than 2 years ago! You might want to consider installing a similar, more up-to-date dependency instead!',
            ),
    ])
        .flatten()
        .compact()
