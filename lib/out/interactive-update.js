...
const savePackages = (packages, saveExact) => {
    const json = JSON.parse(fs.readFileSync('./package.json'));
    const caret = saveExact ? '' : '^';
    for (const pkg of packages) {
        const version = `${caret}${pkg.latest}`;
        json[pkg.devDependency ? 'devDependencies' : 'dependencies'][
            pkg.moduleName
        ] = version;
    }
    if (json.dependencies) json.dependencies = sortDeps(json.dependencies);
    if (json.devDependencies)
        json.devDependencies = sortDeps(json.devDependencies);
...
function interactive(currentState) {
    ...
    return inquirer.prompt(questions).then(answers => {
        ...
        const promise = isJson
            ? Promise.resolve().then(() => {
                  savePackages(packagesToUpdate, saveExact);
                  return currentState;
              })
            : installPackages(saveDependencies, currentState).then(
                  currentState =>
                      installPackages(saveDevDependencies, currentState)
              );
        return promise.then(currentState => {
            console.log('');
            ...
            if (isJson)
                console.log(
                    chalk.red(
                        `[npm-check] You must now run your package manager to install the updates.`
                    )
                );
            console.log(
                chalk.green(
