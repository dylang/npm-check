...
function getNpmInfo(packageName) {
    return q.nfcall(packageJson, packageName)
        .then(function (rawData) {
            ...
            var latest = rawData['dist-tags'].latest; // semver.maxSatisfying(sortedVersions, '*');  //data.version;
            var latestStableRelease = semver.satisfies(latest, '*')  ?
                latest : semver.maxSatisfying(sortedVersions, '*');
            return {
