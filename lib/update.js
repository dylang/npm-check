...
function label(pkg){
    ...
    var homepage = chalk.blue.underline(pkg.homepage);

    return [name + type + missing, installed, installed && '❯', chalk.bold(pkg.latest), homepage];
...
function choice(pkg){
    ...
    return {
        ...
        name: label(pkg),
        checked: (pkg.easy_upgrade || pkg.notInstalled) && pkg.bump !== 'nonSemver'
    };
}
