...
function label(pkg){
    var installed = pkg.notInstalled ? 'missing' : pkg.installed;

    var name = pkg.moduleName + ' '  + spacer(pkg.moduleName, 20);
    var versions = spacer(installed, 7) + chalk.blue(installed) + ' to ' + spacer(pkg.latest, 7) + chalk.blue(pkg.latest);
...
function choice(pkg){
    ...
    return {
        ...
        name: label(pkg),
        checked: pkg.easy_upgrade || pkg.notInstalled
    };
}
