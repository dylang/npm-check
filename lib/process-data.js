...
function processModule(moduleName) {
    const rootPackageJson = options.get('packageJson');
    
    const modulePackageJson = readPackageJson(path.join(options.get('nodeModulesPath'), moduleName, 'package.json'));
    ...
    return getNpmInfo(moduleName)
        .then(data => {
            ...
            const packageJsonVersion = rootPackageJson.dependencies[moduleName] ||
                ...
                options.get('installed')[moduleName];
            ...
            return {
                ...
                isInstalled: options.get('global') || installedVersion,
                notInstalled: !options.get('global') && !installedVersion,
...
function processData() {
    const rootPackageJson = options.get('packageJson');
    const promises = _.map(modulesOfPackage(rootPackageJson), moduleName => {
